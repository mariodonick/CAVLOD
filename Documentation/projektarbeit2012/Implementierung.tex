...
ist \gls{CROP} jedoch nicht f{\"a}hig, einen Verbindungsabbruch zu detektieren).
Die f{\"u}r den Vorgang genutzte Adressierungsart wird im \gls{CROP} festgelegt
...
Map bezeichnet, aus der \gls{STL} zum Einsatz. Die Datenstruktur ist ähnlich zu
einem Vektor, allerdings wird hier jedes Datenfeld mit einem Schlüssel belegt,
anhand dessen die Daten schnell gefunden werden können.
Jeder Schl{\"u}ssel ist einzigartig.\newline
F{\"u}r die innere Map wird die
...
werden. F{\"u}r diese Implementierung wid diese Klasse in dem \gls{GUI}
integriert. Dadurch wird das Darstellen der Zeichen auf dem Textfeld vereinfacht
und es besteht die Möglichkeit bei einer wechselnden \gls{DOID} einen
