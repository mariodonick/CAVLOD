...
einen au{\ss}erirdischen Bereich. Die damit verbundenen {\"A}nderungen im Vergleich zum Irdischen
Internet umfassen z.B. einen gesonderten Umgang mit Latenzen, da diese beim IPN
im Minuten- bis Stundenbereich liegen. Die Entwicklung von Protokollen
f{\"u}r das IPN obliegt dabei dem Consultative Committee for Space Data Systems
(CCSDS).

\textbf{Delay Tolerant Networking}

Delay Tolerant Networking (DTN) bezeichnet eine Protokollarchitektur f{\"u}r
End-to-end Netzwerkverbindungen mit geringer Stabilit{\"a}t. Die Basis der DTN
Netzwerkarchitektur stellt das von der NASA entwickelte IPN dar. Ein wichtiger
...
\todo{layer werden erst später erklärt\ldots}
Das Licklider Transmission Protocol (LTP) kann direkt auf dem Data Link Layer
aufsetzen oder aber auch unter UDP laufen (siehe Abb. \ref{fig:LTP}). LTP wird
zudem als standard convergence layer Protokoll f{\"u}r das Bundle Protokoll
genutzt. Das LTP wurde zur sicheren {\"U}bertragung von Daten zwischen einem
Sender und einem Empf{\"a}nger (point-to-point) unter DTN Bedingungen
entwickelt. LTP entscheidet dabei sogar zwischen wichtigen (red data) und
...
Segmente geteilt. Handelt es sich um wichtige Daten (red data) werden
...
durch den Empf{\"a}nger um so bei einem eventuellen Verbindungsabbruchs eine
erneute Sendung des jeweiligen Datenpakets auszul{\"o}sen. Die Daten werden
solange beim Sender in einer Queue vorgehalten, bis der zugeh{\"o}rige
...
Daten um Daten mit geringerer Relevanz (green data) so ist keine Best{\"a}tigung
durch den Empf{\"a}nger notwendig.
...
interplanetaren Kommunikationsansatz. Dabei repr{\"a}sentiert der linke Teil
des Stacks das Landefahrzeug auf der Marsoberfl{\"a}che, welches das CFDP (CCSDS
